...
export class EthApi extends CustomApi {
    ...
    public constructor(config: EthSetting, mode: string) {
        ...
        var contracts = this._config.contracts instanceof Array ? this._config.contracts : [this._config.contracts];

        for (var contract of contracts) {

            this._web3 && (this._contractMap[contract.name] = this._web3.eth.contract(contract.data).at(contract.address));
            this._web3_browser && (this._contractMap[contract.name] = this._web3_browser.eth.contract(contract.data).at(contract.address));
        }
    ...
    public plugin(): string { return "MateMask" } // 插件名称

    public usePlugin(): boolean { return true; } // 是否使用插件

    public isInitPlugin(): boolean { return this._web3 != undefined; } // 插件是否初始化了

    public hasAccount(): boolean { return this._account != undefined; } // 是否有账号

    public defaultAccount(): string { return this._web3 ? this._web3.eth.defaultAccount : ''; } // 获取默认账号
